# Checklists — Running Decision Processes

Use these checklists before finalizing the Decision Process Pack and at key moments during execution.

## A) Decision Process Pack quality checklist (pre-flight)
- Decision statement is one sentence and includes a deadline.
- One-way door vs two-way door classification is explicit.
- Constraints/non-negotiables are listed (policy/security, budget, timeline, customer promises).
- Options set is 2–4 real options (not strawmen); tradeoffs are visible.
- Evaluation criteria reflect real tradeoffs (not a wish list).
- Key assumptions/unknowns are explicit and testable.
...
- Assumptions are written as statements that could be proven wrong.
- Each major assumption has a plan to test/observe it (metric, leading indicator, pilot).
...
- Owner and first 2–3 milestones are named.
- Review date and metrics are scheduled (calendar + doc link).
- A short retrospective is planned to capture learning and update templates.
