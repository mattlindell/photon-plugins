...
def gap_analysis(
    ...
    for dim in dimensions:
        ...
        gaps[dim] = {
            ...
            "priority": "high"
            if gap_to_best < -2
            else ("medium" if gap_to_best < -1 else "low"),
        }
...
def positioning_analysis(scored: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "your_rank": next(
            (i + 1 for i, c in enumerate(scored) if c.get("is_you")), None
        ),
...
def format_text(result: Dict[str, Any]) -> str:
    ...
    for i, c in enumerate(result["scored_competitors"], 1):
        ...
        lines.append(
            f"{i:<6}{c['name']:<25}{c['overall_score']:<10}{c['tier']:<20}{marker}"
        )
    ...
    if pos:
        lines.append("\n## MARKET POSITIONING\n")
        lines.append(
            f"  Market Leaders: {', '.join(pos.get('market_leaders', ['None']))}"
        )
        if pos.get("your_rank"):
            lines.append(
                f"  Your Rank: #{pos['your_rank']} of {pos['total_competitors']}"
            )
...
def build_matrix(
    data: Dict[str, Any], weight_overrides: Optional[Dict[str, float]] = None
) -> Dict[str, Any]:
