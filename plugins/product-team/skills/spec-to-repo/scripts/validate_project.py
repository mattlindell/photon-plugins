...
def check_readme(root):
    ...
    if size < 50:
        return {
            "name": "readme",
            "status": "WARN",
            "message": f"README.md is only {size} bytes — likely incomplete",
        }
    return {
        "name": "readme",
        "status": "PASS",
        "message": f"README.md exists ({size} bytes)",
    }
...
def check_env_example(root, all_files):
    ...
    if not uses_env:
        return {
            "name": "env_example",
            "status": "PASS",
            "message": "No env vars detected — .env.example not required",
        }
    ...
    if not os.path.isfile(path):
        return {
            "name": "env_example",
            "status": "FAIL",
            "message": "Code references env vars but .env.example is missing",
        }
...
def check_no_env_file(root):
    ...
    if os.path.isfile(path):
        return {
            "name": "no_env_committed",
            "status": "FAIL",
            "message": ".env file found — secrets may be committed",
        }
    return {
        "name": "no_env_committed",
        "status": "PASS",
        "message": "No .env file committed",
    }
    ...
    for manifest in MANIFESTS:
        if os.path.isfile(os.path.join(root, manifest)):
            return {
                "name": "manifest",
                "status": "PASS",
                "message": f"Package manifest found: {manifest}",
            }
    return {
        "name": "manifest",
        "status": "FAIL",
        "message": "No package manifest found (package.json, requirements.txt, go.mod, etc.)",
    }
...
def check_placeholders(all_files, root):
    ...
    if not findings:
        return {
            "name": "placeholders",
            "status": "PASS",
            "message": "No TODO/FIXME/placeholder code found",
        }
    if len(findings) <= 3:
        return {
            "name": "placeholders",
