...
def route_to_page_name(route: str) -> str:
    ...
    return " ".join(
        w.capitalize() for w in "-".join(clean).replace("_", "-").split("-")
    )


def generate_readme(
    project_name: str, routes: List[Dict], summary: Dict, date: str
) -> str:
    ...
    for i, route in enumerate(routes, 1):
        ...
        lines.append(
            f"| {i} | {name} | `{path}` | <!-- TODO --> | [→](./pages/{filename}) |"
        )
...
def generate_enum_dictionary(enums: List[Dict]) -> str:
    ...
    for e in enums:
        ...
        lines.append(
            f"**Type:** {e.get('type', 'unknown')} | **Source:** `{e.get('source', 'unknown').split('/')[-1]}`"
        )
        lines.append("")
        if e.get("values"):
...
def generate_api_inventory(apis: List[Dict]) -> str:
    ...
    unknown = [
        a for a in apis if not a.get("integrated") and not a.get("mock_detected")
    ]

    for label, group in [
        ("Integrated APIs", integrated),
        ("Mock / Stub APIs", mocked),
...
def scaffold(
    analysis: Dict[str, Any], output_dir: Path, project_name: Optional[str] = None
):
    ...
    (appendix_dir / "page-relationships.md").write_text(
        generate_page_relationships(routes)
    )
...
def validate_analysis(analysis: Dict[str, Any]) -> List[str]:
    ...
    if "routes" in analysis:
        ...
        if not isinstance(routes, dict):
            ...
            "pages" not in routes
            and "frontend_pages" not in routes
            and "backend_endpoints" not in routes
        ):
            errors.append(
                "'routes' must contain 'pages', 'frontend_pages', or 'backend_endpoints'"
            )
...
def main():
    ...
    parser.add_argument(
        "analysis", help="Path to analysis JSON from codebase_analyzer.py"
    )
    parser.add_argument(
        "-o", "--output-dir", default="prd", help="Output directory (default: prd/)"
    )
