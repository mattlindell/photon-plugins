...
def analyze_project(project_root: Path) -> Dict[str, Any]:
    ...
    analysis = {
        "project": {
            ...
            "stack_type": "backend"
            if fw in ("django", "fastapi", "flask", "nestjs", "express", "fastify")
...
def format_markdown(analysis: Dict[str, Any]) -> str:
    ...
    lines.append(
        f"**API calls detected:** {summary['api_endpoints']} "
    ...
    lines.append(
        f"**State management:** {'Yes' if summary['has_state_management'] else 'No'}"
    )
    lines.append("")
    ...
    if analysis.get("models", {}).get("definitions"):
        ...
        for m in analysis["models"]["definitions"]:
            ...
            if m.get("fields"):
                ...
                for fld in m["fields"]:
                    lines.append(
                        f"| {fld['name']} | {fld['type']} | {fld.get('args', '')} |"
                    )
            lines.append("")
