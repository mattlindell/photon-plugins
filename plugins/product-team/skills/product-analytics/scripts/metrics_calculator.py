...
def retention(args: argparse.Namespace) -> int:
    ...
    for period in range(0, args.max_period + 1):
        ...
        results.append(
            {"period": period, "active_users": users, "retention_rate": round(rate, 4)}
        )
...
def cohort(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "format", "text") == "json":
        ...
        for r in results:
            print(
                f"{r['cohort']},{r['age_days']},{r['active_users']},{r['cohort_size']},{r['retention_rate']:.4f}"
            )
    return 0
...
def funnel(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "format", "text") == "json":
        ...
        for r in results:
            print(
                f"{r['stage']},{r['users']},{r['conversion_from_previous']:.4f},{r['conversion_from_first']:.4f}"
            )
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    retention_parser = subparsers.add_parser(
        "retention", help="Calculate retention by day."
    )
    ...
    retention_parser.add_argument(
        "--format", choices=["text", "json"], default="text", help=fmt_help
    )
    retention_parser.set_defaults(func=retention)

    cohort_parser = subparsers.add_parser(
        "cohort", help="Build cohort retention matrix rows."
    )
    ...
    cohort_parser.add_argument(
        "--cohort-grain", choices=["week", "month"], default="week"
    )
    ...
    funnel_parser = subparsers.add_parser(
        "funnel", help="Calculate funnel conversion by stage."
    )
