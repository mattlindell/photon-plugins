...
def generate_env_example(config: Dict[str, Any]) -> str:
    ...
    if config.get("auth"):
        lines.extend(
            ["# Auth", "JWT_SECRET=change-me-in-production", "JWT_EXPIRY=7d", ""]
        )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="./my-project", help="Output directory"
    )
    parser.add_argument(
    ...
    if args.format == "json":
        ...
        print(
            f"Project '{result['project_name']}' scaffolded at {result['output_dir']}"
        )
        print(f"Stack: {result['stack']}")
        print(
            f"Created: {result['total_files']} files, {result['total_dirs']} directories"
        )
        if result["dry_run"]:
