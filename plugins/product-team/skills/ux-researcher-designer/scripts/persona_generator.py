...
class PersonaGenerator:
    ...
    def generate_persona_from_data(
        self, user_data: List[Dict], interview_insights: List[Dict] = None
    ) -> Dict:
        ...
        persona = {
            ...
            "psychographics": self._extract_psychographics(
                patterns, interview_insights
            ),
    ...
    def _identify_archetype(self, patterns: Dict) -> str:
        ...
        freq_pattern = (
            max(patterns["usage_frequency"].items(), key=lambda x: x[1])[0]
            if patterns["usage_frequency"]
            else "medium"
        )
        device_pattern = (
            max(patterns["devices"].items(), key=lambda x: x[1])[0]
            if patterns["devices"]
            else "desktop"
        )
        ...
        if freq_pattern == "daily" and len(patterns["feature_usage"]) > 10:
            ...
            return "mobile_first"
        elif patterns["contexts"].get("work", 0) > patterns["contexts"].get(
            "personal", 0
        ):
    ...
    def _generate_tagline(self, patterns: Dict) -> str:
        ...
        context = (
            max(patterns["contexts"].items(), key=lambda x: x[1])[0]
    ...
    def _aggregate_demographics(self, user_data: List[Dict]) -> Dict:
        ...
        # Location type
        locations = [
            u.get("location_type", "urban") for u in user_data if "location_type" in u
        ]
        if locations:
        ...
        # Tech proficiency
        tech_scores = [
            u.get("tech_proficiency", 5) for u in user_data if "tech_proficiency" in u
        ]
        if tech_scores:
    ...
    def _extract_psychographics(
        self, patterns: Dict, interviews: List[Dict] = None
    ) -> Dict:
    ...
    def _analyze_behaviors(self, user_data: List[Dict]) -> Dict:
        ...
        behaviors["usage_patterns"] = [
            f"{freq}: {count} users" for freq, count in freq_counter.most_common(3)
        ]
        ...
        behaviors["feature_preferences"] = [
            feat for feat, count in feature_counter.most_common(5)
        ]
    ...
    def _select_quote(
        self, interviews: List[Dict] = None, archetype: str = "casual_user"
    ) -> str:
    ...
    def _derive_design_implications(self, patterns: Dict) -> List[str]:
        ...
        # Based on frequency
        if patterns["usage_frequency"].get("daily", 0) > patterns[
            "usage_frequency"
        ].get("weekly", 0):
        ...
        # Based on context
        if patterns["contexts"].get("work", 0) > patterns["contexts"].get(
            "personal", 0
        ):
    ...
    def format_persona_output(self, persona: Dict) -> str:
        ...
        if persona["psychographics"]["motivations"]:
            output.append(
                f"  Motivations: {', '.join(persona['psychographics']['motivations'])}"
            )
    ...
    return [
        {
            ...
            "features_used": ["dashboard", "reports", "settings", "sharing", "export"][
                : 3 + (i % 3)
            ],
            "primary_device": ["desktop", "mobile", "tablet"][i % 3],
            "usage_context": ["work", "personal"][i % 2],
            "tech_proficiency": 3 + (i % 7),
            "pain_points": ["slow loading", "confusing UI", "missing features"][
                : (i % 3) + 1
            ],
        }
        for i in range(30)
