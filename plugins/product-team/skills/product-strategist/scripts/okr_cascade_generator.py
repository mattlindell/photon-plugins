...
class OKRGenerator:
    ...
    def generate_okr_dashboard(self, all_okrs: Dict) -> str:
        ...
        dashboard.append(
            f"Product Contribution: {self.product_contribution * 100:.0f}%"
        )
        dashboard.append("=" * 60)
    ...
    def calculate_alignment_score(self, all_okrs: Dict) -> Dict:
        ...
        if total_objectives > 0:
            scores["vertical_alignment"] = round(
                (aligned_objectives / total_objectives) * 100, 1
            )
        ...
        if "company" in all_okrs and "product" in all_okrs:
            company_krs = sum(
                len(obj["key_results"]) for obj in all_okrs["company"]["objectives"]
            )
            covered_krs = sum(
                len(obj["key_results"]) for obj in all_okrs["product"]["objectives"]
            )
            if company_krs > 0:
        ...
        if "teams" in all_okrs:
            objectives_per_team = [
                len(team["objectives"]) for team in all_okrs["teams"]
            ]
            if objectives_per_team:
                avg_objectives = sum(objectives_per_team) / len(objectives_per_team)
                variance = sum(
                    (x - avg_objectives) ** 2 for x in objectives_per_team
...
Examples:
  ...
  python okr_cascade_generator.py operational --teams "Core,Platform" --contribution 0.5 --json
    ...
    parser.add_argument(
        "--json", "-j", action="store_true", help="Output as JSON instead of dashboard"
    )
