...
def parse_inline(items: list[str]) -> list[Assumption]:
    ...
    for item in items:
        ...
        if len(parts) != 4:
            raise ValueError(
                "Inline assumption must be: statement|category|risk|certainty"
            )
        assumptions.append(
            Assumption(
                statement=parts[0],
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Prioritize assumptions and generate test plan."
    )
    ...
    parser.add_argument(
        "--top", type=int, default=10, help="Maximum assumptions to print"
    )
    return parser
