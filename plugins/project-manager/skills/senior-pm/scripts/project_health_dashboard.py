...
class ProjectMetrics:
    ...
    def __init__(self, data: Dict[str, Any]):
        ...
        self.completion_percentage: float = (
            max(0, min(100, data.get("completion_percentage", 0))) / 100
        )
        ...
        # Stakeholder metrics
        self.stakeholder_satisfaction: Optional[float] = data.get(
    ...
    def _calculate_scope_completion(self) -> float:
        ...
        effective_planned = (
            self.planned_features + self.added_features - self.descoped_features
        )
...
def calculate_dimension_score(
    value: float, dimension: str, is_reverse: bool = False
) -> int:
    ...
    if not is_reverse:
        ...
        if value >= thresholds["green"]["min"]:
            ...
            position = (
                (value - thresholds["green"]["min"]) / range_size
                if range_size > 0
                else 1
            )
...
def calculate_project_health_score(project: ProjectMetrics) -> Dict[str, Any]:
    ...
    scope_score = calculate_dimension_score(
        project.scope_health, "scope", is_reverse=True
    )
    quality_score = calculate_dimension_score(
        project.quality_health, "quality", is_reverse=True
    )
    ...
    dimensions = {
        "timeline": {
            ...
            "weight": HEALTH_DIMENSIONS["timeline"]["weight"],
        },
        "budget": {
            "score": budget_score,
            "weight": HEALTH_DIMENSIONS["budget"]["weight"],
        },
        "scope": {"score": scope_score, "weight": HEALTH_DIMENSIONS["scope"]["weight"]},
        "quality": {
            "score": quality_score,
            "weight": HEALTH_DIMENSIONS["quality"]["weight"],
        },
        "risk": {"score": risk_score, "weight": HEALTH_DIMENSIONS["risk"]["weight"]},
    }

    composite_score = sum(
        dim_data["score"] * dim_data["weight"] for dim_data in dimensions.values()
    )
...
def analyze_portfolio_dimensions(
    ...
    avg_scores = {
        dim: data["average_score"] for dim, data in dimension_analysis.items()
    }
...
def _generate_project_recommendations(project: Dict[str, Any]) -> List[str]:
    ...
    if dimension_scores["timeline"]["score"] < 60:
        recommendations.append(
            "Conduct timeline recovery analysis and implement fast-tracking or crashing strategies"
        )

    # Budget recommendations
    if dimension_scores["budget"]["score"] < 60:
        recommendations.append(
            "Implement cost control measures and review budget forecasts"
        )

    # Scope recommendations
    if dimension_scores["scope"]["score"] < 60:
        recommendations.append(
            "Review scope management and consider feature prioritization or descoping"
        )

    # Quality recommendations
    if dimension_scores["quality"]["score"] < 60:
        recommendations.append(
            "Increase testing coverage and implement quality improvement processes"
        )

    # Risk recommendations
    if dimension_scores["risk"]["score"] < 60:
        recommendations.append(
            "Escalate critical risks and implement additional risk mitigation measures"
        )

    # Overall health recommendations
    if project["composite_score"] < 40:
        recommendations.append(
            "Consider project restructuring or emergency stakeholder review"
        )
...
def _identify_risk_factors(project: Dict[str, Any]) -> List[str]:
    ...
    dimension_scores = project["dimension_scores"]
    poor_dimensions = [
        dim for dim, data in dimension_scores.items() if data["score"] < 50
    ]

    if len(poor_dimensions) > 2:
        risk_factors.append(
            f"Multiple failing dimensions: {', '.join(poor_dimensions)}"
        )
...
def generate_portfolio_recommendations(analysis_results: Dict[str, Any]) -> List[str]:
    ...
    if red_percentage > 30:
        recommendations.append(
            "URGENT: 30%+ projects are in red status. Consider portfolio restructuring or resource reallocation."
        )
    elif red_percentage > 15:
        recommendations.append(
            "HIGH: Significant number of projects in red status require immediate attention."
        )

    if amber_percentage > 50:
        recommendations.append(
            "MEDIUM: Over half of portfolio projects need monitoring and support."
        )
    ...
    if weakest_dimension:
        recommendations.append(
            f"Focus improvement efforts on {weakest_dimension} - weakest portfolio dimension."
        )

    # Intervention recommendations
    intervention_list = analysis_results.get("intervention_list", [])
    immediate_count = len(
        [p for p in intervention_list if p["intervention_level"] == "immediate"]
    )
    urgent_count = len(
        [p for p in intervention_list if p["intervention_level"] == "urgent"]
    )

    if immediate_count > 0:
        recommendations.append(
            f"CRITICAL: {immediate_count} projects require immediate intervention within 48 hours."
        )

    if urgent_count > 3:
        recommendations.append(
            f"Capacity alert: {urgent_count} projects need urgent attention - consider resource reallocation."
        )
    ...
    if portfolio_grade == "critical":
        recommendations.append(
            "Portfolio health is critical. Recommend executive review and strategic realignment."
        )
    elif portfolio_grade == "concerning":
        recommendations.append(
            "Portfolio health needs improvement. Implement enhanced monitoring and support."
        )
...
def analyze_portfolio_health(data: Dict[str, Any]) -> PortfolioHealthResult:
    ...
    try:
        ...
        project_scores = [
            calculate_project_health_score(project) for project in projects
        ]
        result.project_scores = project_scores

        # Filter active projects for portfolio analysis
        active_scores = [
            score for i, score in enumerate(project_scores) if projects[i].is_active
        ]
        ...
        if active_scores:
            ...
            result.summary = {
                ...
                "projects_needing_attention": len(
                    [s for s in active_scores if s["composite_score"] < 70]
                ),
                "critical_projects": len(
                    [s for s in active_scores if s["composite_score"] < 40]
                ),
            }
        else:
            result.summary = {
...
def format_text_output(result: PortfolioHealthResult) -> str:
    ...
    lines.append("-" * 30)
    lines.append(
        f"Total Projects: {summary['total_projects']} ({summary.get('active_projects', 0)} active)"
    )

    if "portfolio_average_score" in summary:
        lines.append(
            f"Portfolio Health Score: {summary['portfolio_average_score']:.1f}/100"
        )
        lines.append(
            f"Projects Needing Attention: {summary.get('projects_needing_attention', 0)}"
        )
    ...
    if rag_status:
        ...
        lines.append(
            f"🟢 Green: {rag_counts.get('green', 0)} ({rag_percentages.get('green', 0):.1f}%)"
        )
        lines.append(
            f"🟡 Amber: {rag_counts.get('amber', 0)} ({rag_percentages.get('amber', 0):.1f}%)"
        )
        lines.append(
            f"🔴 Red: {rag_counts.get('red', 0)} ({rag_percentages.get('red', 0):.1f}%)"
        )
        lines.append(
            f"Portfolio Grade: {rag_status.get('portfolio_grade', 'N/A').title()}"
        )
        lines.append("")
    ...
    if dimension_analysis:
        ...
        for dimension, stats in dimension_stats.items():
            lines.append(
                f"{dimension.title()}: {stats['average_score']:.1f} avg "
                f"({stats['projects_below_60']} below 60, {stats['projects_above_80']} above 80)"
            )

        lines.append(
            f"Strongest: {dimension_analysis.get('strongest_dimension', '').title()}"
        )
        lines.append(
            f"Weakest: {dimension_analysis.get('weakest_dimension', '').title()}"
        )
        lines.append("")
    ...
    if intervention_list:
        ...
        if immediate_projects:
            ...
            for project in immediate_projects[:5]:
                lines.append(
                    f"  • {project['project_name']} (Score: {project['composite_score']:.0f})"
                )
    ...
    if result.project_scores:
        top_projects = sorted(
            result.project_scores, key=lambda p: p["composite_score"], reverse=True
        )[:5]
        ...
        for project in top_projects:
            ...
            lines.append(
                f"{status_emoji} {project['project_name']}: {project['composite_score']:.0f}/100"
            )
        lines.append("")
...
ProjectMetrics.calculate_composite_health_score = lambda self: (
    calculate_project_health_score(self)["composite_score"]
)
...
def main() -> int:
    ...
    parser.add_argument("data_file", help="JSON file containing project portfolio data")
    parser.add_argument(
        "--format",
        choices=["text", "json"],
        default="text",
