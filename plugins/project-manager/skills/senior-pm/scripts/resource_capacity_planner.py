...
ROLE_TYPES = {
    ...
    "designer": {
        "hourly_rate": 90,
        "efficiency_factor": 1.0,
        "skill_multipliers": {"ui_design": 1.0, "ux_research": 1.0, "prototyping": 0.9},
    },
    "qa_engineer": {
        "hourly_rate": 80,
...
class Resource:
    ...
    def _calculate_effective_capacity(self):
        ...
        self.available_hours = self.effective_hours_per_week * (
            1 - self.current_utilization
        )

    def _determine_role_config(self):
        """Get role configuration from predefined types."""
        self.role_config = ROLE_TYPES.get(
            self.role,
            {
    ...
    def can_work_on_project(
        self, project_skills: List[str], min_effectiveness: float = 0.6
    ) -> bool:
        """Check if resource can effectively work on project."""
        for skill in project_skills:
            if (
                skill in self.skills
                and self.get_skill_effectiveness(skill) >= min_effectiveness
            ):
                return True
        return False
...
class Project:
    ...
    def __init__(self, data: Dict[str, Any]):
        ...
        self.current_allocation: List[Dict[str, Any]] = data.get(
            "current_allocation", []
        )
    ...
    def _calculate_project_metrics(self):
        ...
        if self.start_date and self.target_end_date:
            try:
                ...
                if self.duration_weeks > 0:
                    self.required_hours_per_week = (
                        self.adjusted_hours / self.duration_weeks
                    )
                else:
        ...
        # Capacity gap
        self.capacity_gap = (
            self.required_hours_per_week - self.currently_allocated_hours
        )
...
def analyze_resource_utilization(resources: List[Resource]) -> Dict[str, Any]:
    ...
    utilization_stats = {
        ...
        "total_allocated": sum(
            r.effective_hours_per_week * r.current_utilization for r in resources
        ),
        "total_available": sum(r.available_hours for r in resources),
    }

    # Calculate overall utilization
    utilization_stats["overall_utilization"] = utilization_stats[
        "total_allocated"
    ] / max(utilization_stats["total_capacity"], 1)
    ...
    for role in role_analysis:
        ...
        role_data["average_utilization"] = 1 - (
            role_data["available_hours"] / max(role_data["total_capacity"], 1)
        )
...
def analyze_project_capacity_requirements(projects: List[Project]) -> Dict[str, Any]:
    ...
    project_stats = {
        ...
        "active_projects": len(
            [p for p in projects if p.status in ["active", "in_progress"]]
        ),
        "planned_projects": len([p for p in projects if p.status == "planned"]),
        "total_estimated_hours": sum(p.adjusted_hours for p in projects),
        "total_weekly_demand": sum(
            p.required_hours_per_week for p in projects if p.status != "completed"
        ),
    }
    ...
    projects_with_gaps = [
        p for p in projects if p.capacity_gap > 0 and p.status != "completed"
    ]
    ...
    return {
        ...
        "capacity_gaps": {
            ...
            "gap_projects": [
                ...
                for p in sorted(
                    projects_with_gaps, key=lambda p: p.capacity_gap, reverse=True
                )[:10]
            ],
        },
...
def optimize_resource_allocation(
    resources: List[Resource], projects: List[Project]
) -> Dict[str, Any]:
    ...
    for project in projects:
        if (
            ...
            and project.current_allocation
        ):
                ...
                if resource:
                    ...
                    for skill in project.required_skills:
                        if skill in resource.skills:
                            avg_skill_effectiveness += resource.get_skill_effectiveness(
                                skill
                            )
                            skill_count += 1
    ...
    under_utilized = [
        r
        for r in resources
        if r.current_utilization < UTILIZATION_THRESHOLDS["under_utilized"]
    ]
    over_allocated_projects = [
        p for p in projects if p.capacity_gap < 0 and p.status != "completed"
    ]
    ...
    for project in projects:
        if project.capacity_gap > 0 and project.status != "completed":
            ...
            suitable_resources.sort(
                key=lambda x: (x["skill_match_score"], x["available_hours"]),
                reverse=True,
            )
...
def simulate_capacity_scenarios(
    resources: List[Resource], projects: List[Project], scenarios: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for scenario in scenarios:
        ...
        scenario_results[scenario_name] = {
            ...
            "resource_utilization": resource_analysis["utilization_stats"][
                "overall_utilization"
            ],
            "total_capacity": resource_analysis["utilization_stats"]["total_capacity"],
            "capacity_gaps": project_analysis["capacity_gaps"][
                "total_gap_hours_weekly"
            ],
            "under_utilized_count": len(
                resource_analysis["utilization_categories"]["under_utilized"]
            ),
            "over_utilized_count": len(
                resource_analysis["utilization_categories"]["over_utilized"]
            ),
...
def _generate_capacity_alerts(
    utilization_categories: Dict[str, List[Resource]],
) -> List[str]:
    ...
    if critical_resources:
        alerts.append(
            f"CRITICAL: {len(critical_resources)} resources are severely over-allocated (>95%)"
        )

    if over_utilized:
        alerts.append(
            f"WARNING: {len(over_utilized)} resources are over-allocated (85-95%)"
        )

    if len(under_utilized) > len(critical_resources) + len(over_utilized):
        alerts.append(
            f"OPPORTUNITY: {len(under_utilized)} resources are under-utilized (<60%)"
        )
...
def _calculate_cost_impact(
    sim_resources: List[Resource], baseline_resources: List[Resource]
) -> float:
    ...
    baseline_cost = sum(
        r.hourly_rate * r.effective_hours_per_week for r in baseline_resources
    )
...
def generate_capacity_recommendations(analysis_results: Dict[str, Any]) -> List[str]:
    ...
    if critical_count > 0:
        recommendations.append(
            f"URGENT: Redistribute workload for {critical_count} critically over-allocated resources to prevent burnout."
        )

    if over_utilized_count > 2:
        recommendations.append(
            f"Consider hiring or redistributing work - {over_utilized_count} team members are over-allocated."
        )

    if under_utilized_count > 0 and critical_count + over_utilized_count > 0:
        recommendations.append(
            f"Rebalance allocation - {under_utilized_count} under-utilized resources could help with over-allocated work."
        )
    ...
    if total_gap > 40:  # More than 1 FTE worth of gap
        recommendations.append(
            f"Capacity shortfall of {total_gap:.0f} hours/week detected. Consider hiring or timeline adjustments."
        )
    ...
    if skill_demand:
        ...
        recommendations.append(
            f"High demand for {top_skill} skills ({top_demand} hours). Consider training or specialized hiring."
        )
    ...
    if efficiency < 0.7:
        recommendations.append(
            "Low allocation efficiency detected. Review skill-to-project matching and consider reallocation."
        )
...
def analyze_capacity(data: Dict[str, Any]) -> CapacityAnalysisResult:
    ...
    try:
        ...
        result.summary = {
            ...
            "overall_utilization": sum(r.current_utilization for r in resources)
            / max(len(resources), 1),
        }
        ...
        result.allocation_optimization = optimize_resource_allocation(
            resources, projects
        )
        ...
        if scenarios:
            result.scenario_analysis = simulate_capacity_scenarios(
                resources, projects, scenarios
            )
...
def format_text_output(result: CapacityAnalysisResult) -> str:
    ...
    lines.append(
        f"Capacity vs Demand: {summary['total_capacity_hours']:.0f}h vs {summary['total_demand_hours']:.0f}h per week"
    )
    ...
    for category, resources in utilization_categories.items():
        if resources:
            lines.append(
                f"{category.replace('_', ' ').title()}: {len(resources)} resources"
            )
            for resource in resources[:3]:  # Show top 3
                lines.append(
                    f"  - {resource['name']} ({resource['role']}): {resource['utilization']:.1%}"
                )
            if len(resources) > 3:
    ...
    lines.append(
        f"Total gap: {capacity_gaps.get('total_gap_hours_weekly', 0):.0f} hours/week"
    )
    ...
    if gap_projects:
        ...
        for project in gap_projects[:5]:
            lines.append(
                f"  - {project['name']} ({project['priority']}): {project['gap_hours']:.0f}h/week gap"
            )
    lines.append("")
...
def format_json_output(result: CapacityAnalysisResult) -> Dict[str, Any]:
    ...
    if "suggested_reallocations" in serialized_result["allocation_optimization"]:
        for suggestion in serialized_result["allocation_optimization"][
            "suggested_reallocations"
        ]:
...
def main() -> int:
    ...
    parser.add_argument(
        "data_file", help="JSON file containing resource and project capacity data"
    )
    parser.add_argument(
        "--format",
        choices=["text", "json"],
