...
PROBABILITY_LEVELS = {
    1: {
        "label": "Very Low",
        "range": "0-10%",
        "description": "Highly unlikely to occur",
    },
...
def build_risk_matrix(risks: List[Risk]) -> Dict[str, Any]:
    ...
    risk_distribution = {
        "critical": len(
            [r for r in risks if r.is_active and r.risk_level == "critical"]
        ),
...
def analyze_risk_categories(risks: List[Risk]) -> Dict[str, Any]:
    ...
    for category, config in RISK_CATEGORIES.items():
        ...
        if category_risks:
            ...
            category_stats[category] = {
                ...
                "top_risks": sorted(
                    category_risks, key=lambda r: r.risk_score, reverse=True
                )[:3],
    ...
    return {
        ...
        "category_concentration": len(
            [c for c in category_stats if category_stats[c]["count"] > 0]
        ),
    }
...
def analyze_mitigation_effectiveness(risks: List[Risk]) -> Dict[str, Any]:
        ...
        if strategy_risks:
            strategy_distribution[strategy] = {
                ...
                "average_risk_score": statistics.mean(
                    [r.risk_score for r in strategy_risks]
                ),
    ...
    return {
        ...
        "top_overdue_risks": sorted(
            overdue_risks, key=lambda r: r.risk_score, reverse=True
        )[:5],
    }


def analyze_risk_trends(
    current_risks: List[Risk], historical_data: Optional[List[Dict]] = None
) -> Dict[str, Any]:
...
def generate_risk_recommendations(
    risks: List[Risk], analysis_results: Dict[str, Any]
) -> List[str]:
    ...
    if critical_risks:
        recommendations.append(
            f"URGENT: Address {len(critical_risks)} critical risks immediately. These require executive attention and dedicated resources."
        )

        for risk in critical_risks[:3]:  # Top 3 critical risks
            recommendations.append(
                f"Critical Risk - {risk.title}: Implement {risk.suggested_approach} strategy within 48 hours."
            )
    ...
    if highest_categories:
        ...
        recommendations.append(
            f"Focus mitigation efforts on {top_category} risks - highest concentration of risk exposure."
        )
    ...
    if coverage < 0.7:
        recommendations.append(
            "Improve mitigation coverage - less than 70% of risks have defined mitigation actions."
        )
    ...
    if avg_score > 15:
        recommendations.append(
            "Portfolio risk exposure is high. Consider scope reduction or additional risk mitigation investments."
        )
    elif avg_score < 8:
        recommendations.append(
            "Risk exposure is well-managed. Consider taking on additional strategic initiatives."
        )
...
def analyze_risks(data: Dict[str, Any]) -> RiskAnalysisResult:
    ...
    try:
        ...
        result.summary = {
            ...
            "critical_risks": len(
                [r for r in active_risks if r.risk_level == "critical"]
            ),
...
def format_text_output(result: RiskAnalysisResult) -> str:
    ...
    lines.append("-" * 30)
    lines.append(
        f"Total Risks: {summary['total_risks']} ({summary['active_risks']} active)"
    )
    lines.append(
        f"Risk Exposure: {summary['total_risk_exposure']:.1f} points (avg: {summary['average_risk_score']:.1f})"
    )
    lines.append(
        f"Critical/High Risks: {summary['critical_risks']}/{summary['high_risks']}"
    )
    ...
    for category, stats in category_stats.items():
        if stats["count"] > 0:
            lines.append(
                f"{category.title()}: {stats['count']} risks, "
    ...
    lines.append(
        f"Average Actions per Risk: {mitigation.get('average_actions_per_risk', 0):.1f}"
    )
    lines.append(
        f"Overdue Mitigations: {mitigation.get('overdue_mitigation_count', 0)} "
    ...
    for i, risk in enumerate(top_risks, 1):
        lines.append(
            f"{i}. {risk.title} (Score: {risk.risk_score:.1f}, Level: {risk.risk_level.title()})"
        )
        lines.append(
            f"   Category: {risk.category.title()}, Strategy: {risk.suggested_approach.title()}"
        )
    lines.append("")
...
def main() -> int:
    ...
    parser.add_argument("data_file", help="JSON file containing risk register data")
    parser.add_argument(
        "--format",
        choices=["text", "json"],
        default="text",
