        ...
        if total_sentiment > 0:
            for sentiment in sentiment_scores:
                sentiment_scores[sentiment] = (
                    sentiment_scores[sentiment] / total_sentiment
                )
...
def analyze_action_item_completion(retros: List[RetrospectiveData]) -> Dict[str, Any]:
    ...
    for priority in ["high", "medium", "low"]:
        priority_items = [
            item for item in all_action_items if item.inferred_priority == priority
        ]
        if priority_items:
    ...
    return {
        ...
        "overdue_rate": len(overdue_items) / len(all_action_items)
        if all_action_items
        else 0.0,
    }
...
def analyze_improvement_trends(retros: List[RetrospectiveData]) -> Dict[str, Any]:
    ...
    metrics_over_time = {
        ...
        "positive_sentiment": [
            r.sentiment_scores.get("positive", 0) for r in sorted_retros
        ],
        "negative_sentiment": [
            r.sentiment_scores.get("negative", 0) for r in sorted_retros
        ],
    ...
    for metric_name, values in metrics_over_time.items():
        if len(values) >= 3:
            trend_analysis[metric_name] = {
                ...
                "latest": values[-1],
                "change_from_first": ((values[-1] - values[0]) / values[0])
                if values[0] != 0
                else 0,
            }
    ...
    for i, retro in enumerate(sorted_retros):
        if i > 0:  # Skip first retro as it has no previous action items to complete
            ...
            if prev_retro.action_items:
                completed_count = sum(
                    1
                    for item in prev_retro.action_items
                    if item.is_completed
                    and item.completed_sprint == retro.sprint_number
                )
    ...
    if completion_rates_by_sprint:
        trend_analysis["action_item_completion"] = {
            ...
            "latest": completion_rates_by_sprint[-1]
            if completion_rates_by_sprint
            else 0,
        }
    ...
    return {
        ...
        "retrospective_quality_trend": _assess_retrospective_quality_trend(
            sorted_retros
        ),
...
def _calculate_team_maturity(retros: List[RetrospectiveData]) -> Dict[str, Any]:
    ...
    # Sentiment balance
    avg_positive = sum(r.sentiment_scores.get("positive", 0) for r in retros) / len(
        retros
    )
    avg_negative = sum(r.sentiment_scores.get("negative", 0) for r in retros) / len(
        retros
    )
    ...
    avg_recent_actions = sum(len(r.action_items) for r in recent_retros) / len(
        recent_retros
    )

    if (
        avg_recent_actions <= 3
    ):  # Fewer action items might indicate better follow-through
    ...
    return {"score": overall_score, "level": level, "indicators": maturity_indicators}


def _assess_retrospective_quality_trend(
...
def _calculate_improvement_velocity(retros: List[RetrospectiveData]) -> Dict[str, Any]:
    ...
    if len(retros) >= 4:
        ...
        if early_action_density > 0:
            action_efficiency = min(
                1.0, early_action_density / max(recent_action_density, 1)
            )
    else:
...
def generate_recommendations(result: RetroAnalysisResult) -> List[str]:
    ...
    if completion_rate < 0.5:
        recommendations.append(
            "CRITICAL: Low action item completion rate (<50%). Reduce action items per retro and focus on realistic, achievable goals."
        )
    elif completion_rate < 0.7:
        recommendations.append(
            "Improve action item follow-through. Consider assigning owners and due dates more systematically."
        )
    elif completion_rate > 0.9:
        recommendations.append(
            "Excellent action item completion! Consider taking on more ambitious improvement initiatives."
        )

    overdue_rate = action_analysis.get("overdue_rate", 0)
    if overdue_rate > 0.3:
        recommendations.append(
            "High overdue rate suggests unrealistic timelines. Review estimation and prioritization process."
        )
    ...
    if len(persistent_issues) >= 2:
        recommendations.append(
            f"Address {len(persistent_issues)} persistent issues that keep recurring across retrospectives."
        )
        for issue in persistent_issues[:2]:  # Top 2 issues
            recommendations.append(
                f"Focus on resolving recurring {issue['theme']} issues (appears in {issue['frequency']:.0%} of retros)."
            )
    ...
    if "team_maturity_score" in improvement_trends:
        ...
        if level == "forming":
            recommendations.append(
                "Team is in forming stage. Focus on establishing basic retrospective disciplines and psychological safety."
            )
        elif level == "developing":
            recommendations.append(
                "Team is developing. Work on action item follow-through and deeper root cause analysis."
            )
        elif level == "performing":
            recommendations.append(
                "Good team maturity. Consider advanced techniques like continuous improvement tracking."
            )
        elif level == "high_performing":
            recommendations.append(
                "Excellent retrospective maturity! Share practices with other teams and focus on innovation."
            )
    ...
    if "retrospective_quality_trend" in improvement_trends:
        ...
        if avg_quality < 60:
            recommendations.append(
                "Retrospective quality is below average. Review facilitation techniques and engagement strategies."
            )

        trend_direction = quality_trend.get("trend", {}).get("direction", "stable")
        if trend_direction == "decreasing":
            recommendations.append(
                "Retrospective quality is declining. Consider changing facilitation approach or addressing team engagement issues."
            )
    ...
    try:
        ...
        result.summary = {
            ...
            "date_range": {
                ...
                "span_sprints": retros[-1].sprint_number - retros[0].sprint_number + 1
                if retros
                else 0,
            },
            "average_duration": statistics.mean(
                [r.duration_minutes for r in retros if r.duration_minutes > 0]
            ),
...
def format_text_output(result: RetroAnalysisResult) -> str:
    ...
    lines.append(
        f"Average Completion Time: {action_analysis.get('average_completion_time', 0):.1f} sprints"
    )
    lines.append(
        f"Overdue Items: {action_analysis.get('overdue_items', 0)} ({action_analysis.get('overdue_rate', 0):.1%})"
    )
    ...
    if priority_analysis:
        ...
        for priority, data in priority_analysis.items():
            lines.append(
                f"  {priority.title()}: {data['completion_rate']:.1%} ({data['completed']}/{data['total']})"
            )
    lines.append("")
    ...
    if recurring_themes:
        ...
        sorted_themes = sorted(
            recurring_themes.items(), key=lambda x: x[1]["frequency"], reverse=True
        )
        for theme, data in sorted_themes[:5]:
            lines.append(
                f"  {theme.replace('_', ' ').title()}: {data['frequency']:.1%} frequency, {data['trend']['direction']} trend"
            )
    ...
    if persistent_issues:
        ...
        for issue in persistent_issues:
            lines.append(
                f"  {issue['theme'].replace('_', ' ').title()}: {issue['frequency']:.1%} frequency"
            )
    lines.append("")
    ...
    if "improvement_velocity" in improvement_trends:
        ...
        lines.append(
            f"Theme Resolution Rate: {velocity.get('theme_resolution_rate', 0):.1%}"
        )
        lines.append("")
...
def main() -> int:
    ...
    parser.add_argument("data_file", help="JSON file containing retrospective data")
    parser.add_argument(
        "--format",
        choices=["text", "json"],
        default="text",
