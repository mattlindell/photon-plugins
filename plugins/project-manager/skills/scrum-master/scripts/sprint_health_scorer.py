...
class SprintHealthData:
    ...
    def _calculate_derived_metrics(self):
        ...
        self.commitment_ratio = self.completed_points / max(self.planned_points, 1)
...
def score_commitment_reliability(sprints: List[SprintHealthData]) -> Dict[str, Any]:
    ...
    consistency = 1.0 - (
        statistics.stdev(commitment_ratios) if len(commitment_ratios) > 1 else 0
    )
...
def score_story_completion_distribution(
    sprints: List[SprintHealthData],
) -> Dict[str, Any]:
    ...
    for sprint in sprints:
        if sprint.stories:
            ...
            story_analysis["completed_stories"] += sum(
                1 for s in sprint.stories if s.is_completed
            )
            story_analysis["blocked_stories"] += sum(
                1 for s in sprint.stories if s.is_blocked
            )
    ...
    if story_analysis["total_stories"] > 0:
        blocked_ratio = (
            story_analysis["blocked_stories"] / story_analysis["total_stories"]
        )
...
def score_velocity_predictability(sprints: List[SprintHealthData]) -> Dict[str, Any]:
    ...
    if len(sprints) < 2:
        return {
            "score": 50,
            "grade": "fair",
            "details": "Insufficient sprints for predictability analysis",
        }
...
def _generate_detailed_metrics(sprints: List[SprintHealthData]) -> Dict[str, Any]:
    ...
    if team_sizes:
        metrics["team_metrics"] = {
            ...
            "team_size_stability": statistics.stdev(team_sizes)
            if len(team_sizes) > 1
            else 0,
        }
    ...
    if all_stories:
        ...
        metrics["story_metrics"] = {
            ...
            "average_story_points": statistics.mean(story_points)
            if story_points
            else 0,
    ...
    if all_blockers:
        resolution_times = [
            b.get("resolution_days", 0)
            for b in all_blockers
            if b.get("resolution_days", 0) > 0
        ]
        metrics["blocker_metrics"] = {
            "total_blockers": len(all_blockers),
            "resolved_blockers": len(resolution_times),
            "average_resolution_days": statistics.mean(resolution_times)
            if resolution_times
            else 0,
        }
...
def _generate_health_recommendations(result: HealthScoreResult) -> List[str]:
    ...
    if result.overall_score < OVERALL_HEALTH_THRESHOLDS["poor"]:
        recommendations.append(
            "CRITICAL: Sprint health is poor across multiple dimensions. Immediate intervention required."
        )
    elif result.overall_score < OVERALL_HEALTH_THRESHOLDS["fair"]:
        recommendations.append(
            "Sprint health needs improvement. Focus on top 2-3 problem areas."
        )
    elif result.overall_score >= OVERALL_HEALTH_THRESHOLDS["excellent"]:
        recommendations.append(
            "Excellent sprint health! Maintain current practices and share learnings with other teams."
        )
        ...
        if isinstance(scores, dict) and "score" in scores:
            ...
            if score < 50:  # Poor performance
                if dimension == "commitment_reliability":
                    recommendations.append(
                        "Improve sprint planning accuracy and realistic capacity estimation."
                    )
                elif dimension == "scope_stability":
                    recommendations.append(
                        "Reduce mid-sprint scope changes. Strengthen backlog refinement process."
                    )
                elif dimension == "blocker_resolution":
                    recommendations.append(
                        "Implement faster blocker escalation and resolution processes."
                    )
                elif dimension == "ceremony_engagement":
                    recommendations.append(
                        "Improve ceremony facilitation and team engagement strategies."
                    )
                elif dimension == "story_completion_distribution":
                    recommendations.append(
                        "Focus on completing stories fully rather than starting many partially."
                    )
                elif dimension == "velocity_predictability":
                    recommendations.append(
                        "Work on consistent estimation and delivery patterns."
                    )

            elif score >= 85:  # Excellent performance
                dimension_name = dimension.replace("_", " ").title()
                recommendations.append(
                    f"Excellent {dimension_name}! Document and share best practices."
                )
...
def format_text_output(result: HealthScoreResult) -> str:
    ...
    for dimension, scores in result.dimension_scores.items():
        if isinstance(scores, dict) and "score" in scores:
            ...
            lines.append(
                f"  Score: {scores['score']:.1f}/100 ({scores['grade'].title()})"
            )
    ...
    if metrics:
        ...
        if "blocker_metrics" in metrics and metrics["blocker_metrics"]:
            ...
            lines.append(
                f"Average Resolution Time: {blockers.get('average_resolution_days', 0):.1f} days"
            )
...
def main() -> int:
    ...
    parser.add_argument("data_file", help="JSON file containing sprint health data")
    parser.add_argument(
        "--format",
        choices=["text", "json"],
        default="text",
