...
class SprintData:
    ...
    def __init__(self, data: Dict[str, Any]):
        ...
        self.commitment_ratio: float = self.completed_points / max(
            self.planned_points, 1
        )
        self.scope_change_ratio: float = (
            self.added_points + self.removed_points
        ) / max(self.planned_points, 1)
...
def calculate_rolling_averages(
    sprints: List[SprintData], window_sizes: List[int] = [3, 5, 8]
) -> Dict[int, List[float]]:
...
def detect_trend(
    sprints: List[SprintData], lookback_sprints: int = 6
) -> Dict[str, Any]:
    ...
    recent_sprints = (
        sprints[-lookback_sprints:] if len(sprints) > lookback_sprints else sprints
    )
        ...
        anomalies.append(
            {
                ...
                "expected_range": (
                    mean_velocity - 2 * std_dev,
                    mean_velocity + 2 * std_dev,
                ),
                "z_score": z_score,
                "anomaly_type": anomaly_type,
                "deviation_percentage": (
                    (sprint.velocity - mean_velocity) / mean_velocity
                )
                * 100,
            }
        )
...
def monte_carlo_forecast(
    sprints: List[SprintData], sprints_ahead: int = 6
) -> Dict[str, Any]:
...
def generate_recommendations(analysis: VelocityAnalysis) -> List[str]:
    ...
    if trend == "strong_decline":
        recommendations.append(
            "URGENT: Address strong declining velocity trend. Review impediments, team capacity, and story complexity."
        )
    elif trend == "decline":
        recommendations.append(
            "Monitor declining velocity. Consider impediment removal and capacity planning review."
        )
    elif trend == "strong_improvement":
        recommendations.append(
            "Excellent improvement trend! Document successful practices to maintain momentum."
        )
    ...
    if volatility == "very_high":
        recommendations.append(
            "HIGH PRIORITY: Reduce velocity volatility. Review story sizing, definition of done, and sprint planning process."
        )
    elif volatility == "high":
        recommendations.append(
            "Work on consistency. Review estimation practices and sprint commitment process."
        )
    elif volatility == "low":
    ...
    if len(analysis.anomalies) > 0:
        ...
        if extreme_anomalies:
            recommendations.append(
                f"Investigate {len(extreme_anomalies)} extreme velocity anomalies for root causes."
            )
    ...
    if avg_commitment < 0.8:
        recommendations.append(
            "Low sprint commitment achievement. Review capacity planning and story complexity estimation."
        )
    elif avg_commitment > 1.2:
        recommendations.append(
            "Consistently over-committing. Consider more realistic sprint planning."
        )
    ...
    try:
        ...
        analysis.summary = {
            ...
            "commitment_analysis": {
                ...
                "commitment_consistency": statistics.stdev(commitment_ratios)
                if len(commitment_ratios) > 1
                else 0,
            ...
            "scope_change_analysis": {
                ...
                "scope_change_volatility": statistics.stdev(scope_change_ratios)
                if len(scope_change_ratios) > 1
                else 0,
            },
...
def format_text_output(analysis: VelocityAnalysis) -> str:
    ...
    lines.append(
        f"Velocity Range: {velocity_stats.get('min', 0)} - {velocity_stats.get('max', 0)} points"
    )
    ...
    lines.append("-" * 30)
    lines.append(
        f"Volatility Level: {volatility.get('volatility', 'Unknown').replace('_', ' ').title()}"
    )
    lines.append(
        f"Coefficient of Variation: {volatility.get('coefficient_of_variation', 0):.2%}"
    )
    lines.append(
        f"Standard Deviation: {volatility.get('standard_deviation', 0):.1f} points"
    )
    lines.append("")
    ...
    lines.append("-" * 30)
    lines.append(
        f"Trend Direction: {trend_analysis.get('trend', 'Unknown').replace('_', ' ').title()}"
    )
    lines.append(f"Trend Confidence: {trend_analysis.get('confidence', 0):.1%}")
    lines.append(
        f"Velocity Change Rate: {trend_analysis.get('relative_slope', 0):.1%} per sprint"
    )
    lines.append("")
    ...
    if "error" not in forecasting:
        lines.append(
            f"Expected Total: {forecasting.get('expected_total', 0):.0f} points"
        )
        lines.append(
            f"Average Per Sprint: {forecasting.get('average_per_sprint', 0):.1f} points"
        )
        ...
        lines.append(
            f"Forecast unavailable: {forecasting.get('error', 'Unknown error')}"
        )
    lines.append("")
    ...
    if analysis.anomalies:
        ...
        for anomaly in analysis.anomalies:
            lines.append(
                f"Sprint {anomaly['sprint_number']} ({anomaly['sprint_name']})"
            )
...
def main() -> int:
    ...
    parser.add_argument("data_file", help="JSON file containing sprint data")
    parser.add_argument(
        "--format",
        choices=["text", "json"],
        default="text",
