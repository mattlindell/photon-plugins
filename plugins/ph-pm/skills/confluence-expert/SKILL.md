...
`ph-pm/references/atlassian-mcp-tools.md` — read it before any MCP call, and never invent a tool name. Get the
...
- **Jira Expert** (`ph-pm/jira-expert/`) — the JQL and issue linking behind embedded Jira macros
- **Atlassian Templates** (`ph-pm/atlassian-templates/`) — org-wide template lifecycle and storage-format
  template bodies
