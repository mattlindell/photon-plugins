...
Before handing back, if the run touched a one-way door or the trail will be committed for a reviewer, spawn one subagent on a different model family from the one that did the work. For a local trail on reversible work, audit it yourself and skip the agent. Self-review is not a substitute. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
