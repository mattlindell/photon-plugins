...
Inside a `dispatch` run, the **Babysit** playbook ([`../dispatch/playbooks/babysit.md`](../dispatch/playbooks/babysit.md)) supersedes this skill: it owns mode declaration, the merge frontier, stack safety, and the `watch-pr` watcher. This skill stays the standalone `/babysit` entry point for a single PR outside a `dispatch` run.

## When to use

- There's an open PR and the user explicitly wants it kept green, and you are not already inside a `dispatch` run (the playbook owns that case).
...
2. **Triage in priority order.**
   ...
   - Review-bot comments (Bugbot and similar automation): classify fix/dismiss/ask before acting, per [`bugbot-triage.md`](../dispatch/references/bugbot-triage.md). Ask by default on security, data, and high-severity findings.
...
- Opening a PR does not start a babysit; inside a `dispatch` run the Babysit playbook owns the request and starts only when asked.
