...
- `model`: your configured why-investigators model (default in [Fan-out](#fan-out))
...
- `subagent_type`: `general-purpose`
- `model`: your configured why-synthesizer model (default in [Fan-out](#fan-out))
...
| Role | Tier | Cap |
| --- | --- | --- |
| Investigators | `scan` | 4 |
| Synthesizer | `judgment` | 1 |

One investigator per evidence source, to a maximum of four — pick the four most
likely to hold the answer rather than sweeping every available source. Null
results are still findings.
